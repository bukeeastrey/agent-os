...
class SessionStorage:
    ...
    async def delete_session(self, session_key: str) -> None:
        ...
        # Scope the child deletes by session_key as well as the current
        # session_id. upsert_session is ON CONFLICT(session_key) DO UPDATE SET
        # session_id=excluded.session_id, so one key can outlive several ids;
        # deleting by the current id alone leaves every row written under a
        # previous one behind, and a deterministic key hands those orphans to
        # the next session that reuses it. session_id stays in the predicate so
        # any legacy row whose session_key was never populated still goes.
        await self.conn.execute("BEGIN IMMEDIATE")
        try:
            await self.conn.execute(
                "DELETE FROM transcript_entries WHERE session_key = ? OR session_id = ?",
                (session_key, session.session_id),
            )
            await self.conn.execute(
                "DELETE FROM compacted_transcript_entries WHERE session_key = ? OR session_id = ?",
                (session_key, session.session_id),
            )
            await self.conn.execute(
                "DELETE FROM session_summaries WHERE session_key = ? OR session_id = ?",
                (session_key, session.session_id),
            )
            await self.conn.execute(
                "DELETE FROM session_context_states WHERE session_key = ? OR session_id = ?",
                (session_key, session.session_id),
            )
            await self.conn.execute(
